...
@property (nonatomic, strong) ButtonClickHandler handler;
...
- (void)setup {
    ...
    constraints = [NSLayoutConstraint constraintsWithVisualFormat:@"H:|-10-[imageButton]-10-|" options:0 metrics:nil views:views];
    [self addConstraints:constraints];
...
@property (nonatomic, assign) int workingNumberOfButtonPerLines;
// 按钮间的间隔大小
@property (nonatomic, assign) CGFloat buttonSpace;
// 消失的时候移除
@property (nonatomic, strong) NSLayoutConstraint *contentViewAndViewConstraint;
// iconView的高度constraint
@property (nonatomic, strong) NSLayoutConstraint *iconViewHeightConstraint;
// ButtonView的constraint
@property (nonatomic, strong) NSMutableArray *buttonConstraintArray;
...
- (void)setup {
    ...
    self.buttonConstraintArray = [NSMutableArray array];
    self.lines = 0;
...
- (void)updateConstraints {
    ...
    NSArray *constraints = nil;
    NSLayoutConstraint *constraint = nil;
    NSDictionary *views = @{@"contentView":self.contentView,@"closeButton":self.closeButton,@"cancelButton":self.cancelButton,@"iconView":self.iconView,@"view":self,@"referView":self.referView};
    
    // referView 和 View 的宽高相等
    ...
    constraints = [NSLayoutConstraint constraintsWithVisualFormat:@"V:|-8-[iconView]-[cancelButton(==50)]-8-|" options:0 metrics:nil views:views];
    [self.contentView addConstraints:constraints];
...
- (void)prepareForShow {
    ...
    if (count % self.workingNumberOfButtonPerLines != 0) {
        self.lines++;
    }
    ...
    for (int i = 0; i < _buttonArray.count; i++) {
        ...
        int y = i/self.workingNumberOfButtonPerLines;
        int x = i%self.workingNumberOfButtonPerLines;
        ...
        [self.buttonConstraintArray addObject:constraint];
...
- (void)deviceRotate:(NSNotification*)notification {
    [self.iconView removeConstraints:self.buttonConstraintArray];
    [self.buttonConstraintArray removeAllObjects];
...
- (void)calculateButtonSpaceWithNumberOfButtonPerLine:(int)number
{
    ...
    if (self.buttonSpace < 0) {
        ...
        self.workingNumberOfButtonPerLines = number;
